...
class DataAugmentForObjectDetection():
    ...
    def _cutout(self, img, bboxes, length=100, n_holes=1, threshold=0.5):
        ...
        for n in range(n_holes):
            try_time = 0 # 尝试次数，超过10次就continue，避免卡住
            mask_success = True
            chongdie = True    #看切割的区域是否与box重叠太多
            
            while chongdie:
                try_time += 1
                if try_time > 10: # 如果超过10次
                    mask_success =False
                    break
        ...
        print('mask_success:{}'.format(mask_success))
        if mask_success:
            img = img * mask
...
if __name__ == '__main__':
    ...
    source_pic_root_path = '/home/w61/GItCodes/CV_ToolBox/new/ori_img'
    source_xml_root_path = '/home/w61/GItCodes/CV_ToolBox/new/ori_xml'

    target_pic_root_path = '/home/w61/GItCodes/CV_ToolBox/new/new'
    target_xml_root_path = '/home/w61/FireDetection/Fire_dataset/fire1000_aug/Annotations'
    ...
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            ...
            while cnt < need_aug_num:
                ...
                auged_img, auged_bboxes = dataAug.dataAugment(img, coords) # 替换后的图
                for i,bbox in enumerate(auged_bboxes): # 把bbox信息扩充为[[x_min,y_min,x_max,y_max,name]]
                    bbox = bbox.append(names[i])
                cnt += 1

                # show_pic(auged_img, auged_bboxes)  # 强化后的图
                target_img_path = os.path.join(target_pic_root_path,file)
                # target_xml_path = os.path.join(target_xml_path,file[:-4]+'.xml')

                cv2.imwrite(target_img_path,auged_img) # 写图片
                #generate_xml(file,auged_bboxes,auged_img.shape,target_xml_root_path,target_pic_root_path) #　写xml
